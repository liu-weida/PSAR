...
public class ServerProcessor {
    ...
    public Message process(Message message) throws IOException {
        ...
        switch (clientMessage.getCommand()) {
            ...
            case "dAccessWrite" -> message = handleDAccessWrite(variableId, InetAddress.getLocalHost(), clientPort);
            ...
            default -> message = new ServerMessage(MessageType.EXP, OperationStatus.COMMAND_ERROR, variableId);
        }
        return message;
    }
    ...
    private ServerMessage handleDMalloc(String variableId) {
        ...
        if (server.variableExistsHeap(variableId)) {
            return new ServerMessage(MessageType.DMA, OperationStatus.DATA_EXISTS, variableId);
        } else {
            //尝试在服务器堆中添加数据信息
            server.modifyHeapDMalloc(variableId);
            return new ServerMessage(MessageType.DMA, OperationStatus.SUCCESS, variableId);
        }
    }
    ...
    private ServerMessage handleDAccessWrite(String variableId, InetAddress host, int port) {
        ...
        if (!server.variableExistsHeap(variableId))
            return new ServerMessage(MessageType.DAW, OperationStatus.DATA_NOT_EXISTS, variableId);

        switch (server.modifyHeapDAccessWrite(variableId, host, port)) {
            case SUCCESS -> { return new ServerMessage(MessageType.DAW, OperationStatus.SUCCESS, variableId); } //没锁
            case LOCKED -> { return new ServerMessage(MessageType.DAW, OperationStatus.LOCKED, variableId); } //锁了
            default -> { return new ServerMessage(MessageType.DRE, OperationStatus.ERROR, variableId); }
        }
    }
    ...
    private Message handleDAccessRead(String variableId) {
        ...
        if (!server.variableExistsHeap(variableId))
            return new ServerMessage(MessageType.DAR, OperationStatus.DATA_NOT_EXISTS, variableId);

        switch ((OperationStatus) server.modifyHeapDAccessRead(variableId).first()) {
            case SUCCESS -> {
                Pair p = (Pair) server.modifyHeapDAccessRead(variableId).second();
                return new ServerMessage(MessageType.DAR, OperationStatus.SUCCESS,(InetAddress) p.first(), (Integer) p.second(), variableId);
            }
            case LOCKED -> { return new ServerMessage(MessageType.DAR, OperationStatus.LOCKED, variableId); }
            default -> { return new ServerMessage(MessageType.DAR, OperationStatus.ERROR, variableId); }
        }
    ...
    private ServerMessage handleDRelease(String variableId) throws IOException {
        ...
        switch (server.modifyHeapDRelease(variableId)) {
            case SUCCESS -> { return new ServerMessage(MessageType.DRE, OperationStatus.SUCCESS, variableId); }
            default -> { return new ServerMessage(MessageType.DRE, OperationStatus.ERROR, variableId); }
        }
    }
    ...
    private ServerMessage handleDFree(String variableId) throws IOException {
        ...
        if (! server.variableExistsHeap(variableId))
            return new ServerMessage(MessageType.DFR, OperationStatus.DATA_NOT_EXISTS, variableId);

        switch (server.modifyHeapDFree(variableId)) {
            case SUCCESS -> { return new ServerMessage(MessageType.DFR, OperationStatus.SUCCESS, variableId); }
            case LOCKED -> { return new ServerMessage(MessageType.DFR, OperationStatus.LOCKED, variableId); }
            default -> { return new ServerMessage(MessageType.DRE, OperationStatus.ERROR, variableId); }
        }
    }
}